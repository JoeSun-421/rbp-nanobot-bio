...
Generic assistant tools (shell/web/filesystem/…) remain on disk for Config schema
lazy defaults / hard imports, but are never registered by default.
"""
...
_SKIP_MODULES = frozenset({
    ...
    "path_utils", "exec_session",
})

# Proposal §8 lightweight core: RBP toolkit only unless explicitly opted out.
_ALLOW_MODULES = frozenset(
    m.strip()
    for m in os.environ.get("NANOBOT_TOOL_ALLOW", "rbp").split(",")
    if m.strip()
)
...
class ToolLoader:
    ...
    def discover(self) -> list[type[Tool]]:
        ...
        for _importer, module_name, _ispkg in pkgutil.iter_modules(self._package.__path__):
            if module_name.startswith("_") or module_name in _SKIP_MODULES:
                continue
            # Slim vendor: only allowlisted modules (default: rbp).
            if _ALLOW_MODULES and module_name not in _ALLOW_MODULES:
                continue
            try:
    ...
    def load(self, ctx: Any, registry: ToolRegistry, *, scope: str = "core") -> list[str]:
        ...
        # Slim vendor: skip entry-point plugins unless explicitly enabled.
        allow_plugins = os.environ.get("NANOBOT_TOOL_PLUGINS", "").strip().lower() in (
            "1", "true", "yes", "on",
        )
