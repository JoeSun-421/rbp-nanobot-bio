...
class OpenAICompatProvider(LLMProvider):
    ...
    def _build_client(self) -> None:
        ...
        # Disable HTTP keepalive for all OpenAI-compatible endpoints.
        # Long tool rounds (AF3 / predict / etc.) often outlive the provider's
        # idle timeout; the next LLM call then hits a dead pooled connection →
        # "APIConnectionError: connection error" on attempt 1/3. Fresh connects
        # are cheap relative to agent turns. For local endpoints also force
        # proxy=None so HTTP_PROXY cannot hijack localhost.
        _limits = httpx.Limits(keepalive_expiry=0)
        if self._is_local:
            http_client = httpx.AsyncClient(
                limits=_limits,
                timeout=timeout_s,
                transport=httpx.AsyncHTTPTransport(proxy=None, limits=_limits),
            )
        else:
            http_client = httpx.AsyncClient(limits=_limits, timeout=timeout_s)
