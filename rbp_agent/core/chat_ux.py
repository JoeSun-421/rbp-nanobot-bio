...
def print_status_panel(
    *,
    ...
    s = Style(stream)
    from rbp_agent.core.paths import ARTIFACTS, PACKAGE_ROOT
...
def configure_chat_logging(*, verbose: bool = False, log_file: Optional[Path] = None) -> None:
    ...
    if log_file is None:
        from rbp_agent.core.paths import DEFAULT_CHAT_LOG, ensure_artifact_dirs
...
def _init_prompt_session(history_dir: Optional[Path] = None) -> None:
    ...
    if history_dir is None:
        from rbp_agent.core.paths import SESSIONS, ensure_artifact_dirs
