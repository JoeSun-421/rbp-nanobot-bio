...
def run_gate(*, skip_eval: bool = False, with_cov: bool = True) -> int:
    ...
    from rbp_agent.core.paths import REPORTS, ensure_artifact_dirs

    ensure_artifact_dirs()
    failures: list[str] = []

    if _run([sys.executable, "-m", "ruff", "check", "rbp_agent", "rbp_eval", "tests"]):
    ...
    lay = subprocess.call(
        [sys.executable, "-m", "rbp_agent", "layout"],
        cwd=str(ROOT),
        env=env,
    )
    if lay:
    ...
    if skip_eval:
        ...
        if _run([sys.executable, "-m", "rbp_agent", "eval-plan", "--out", str(plan_out)]):
