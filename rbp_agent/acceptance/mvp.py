...
def main() -> int:
    try:
        from rbp_agent.core.chat_ux import configure_chat_logging
    ...
    c = add("A", "rbp_eval + rbp_agent + artifacts")
    ok_files = all(
        (ROOT / p).exists()
        for p in (
            "rbp_eval/runner.py",
    ...
    if ok_files:
        c.pass_("SoT nanobot/ + rbp_eval/ + rbp_agent/")
    else:
    ...
    try:
        from rbp_agent.integrate import RBPAgent, skill_path
    ...
    try:
        from rbp_agent.core.verdict_schema import validate_verdict, normalize_verdict
    ...
    try:
        ...
        from rbp_agent.backends.delivery.registry import STAGE_RAW_WHITELIST, register_tools
        from rbp_agent.backends.delivery.client import DeliveryToolClient
        import asyncio
...
if __name__ == "__main__":
    ...
    from rbp_agent.backends.delivery.env import delivery_root  # noqa: F401
