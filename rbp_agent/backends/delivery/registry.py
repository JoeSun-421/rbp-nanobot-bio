...
Canonical import: ``from rbp_agent.backends.delivery.registry import register_tools``.
"""
...
from rbp_agent.backends.delivery.client import (  # noqa: E402
    DeliveryToolClient,
    SCRIPT_MAP,
    tools_meta_by_name,
)
from rbp_agent.backends.delivery.env import apply_delivery_env  # noqa: E402

# Stage 0 / 3 extras for MVP agent (curated P0–P2 registered separately).
...
def _normalize_delivery_payload(name: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if name == "similarity_weighted_vote":
        ...
        try:
            from rbp_agent.core.runtime_config import abstain_thresholds
...
def build_proposal_tools(client: Optional[DeliveryToolClient] = None) -> list[Tool]:
    ...
    try:
        ...
        try:
            from rbp_agent.integrate import install_rbp_tools_into_nanobot
