...
C. Eval-only fixture pipeline (informational; NOT proposal MVP)
D. Near-match helpers + self-evolution modules
E. nanobot Tool registration + execute (whitelist)
F. Nanobot.from_config().run → **formal proposal MVP**

Hard gate: A + B + D. C never gates MVP.
Exit 0 if hard gate (+ preferably E). Full proposal needs F.
"""
...
def main() -> int:
    try:
        from core.chat_ux import configure_chat_logging
    ...
    # ----- A: structure -----
    c = add("A", "proposal nanobot/agent/tools/rbp")
    ...
    c = add("A", "rbp_eval runner+evaluator+traces")
    ok_files = all(
        (ROOT / p).exists()
        for p in (
            "rbp_eval/runner.py",
    ...
    try:
        from core.verdict_schema import validate_verdict, normalize_verdict
    ...
    try:
        ...
        from backends.delivery.registry import STAGE_RAW_WHITELIST, register_tools
        from backends.delivery.client import DeliveryToolClient
        import asyncio
    ...
    # ----- F: formal proposal MVP -----
    c = add("F", "Nanobot.from_config().run (formal proposal MVP)")
    try:
        ...
        result = agent.run_sync(msg, session_key="mvp:llm")
    ...
    # C is eval fixture only — never a hard gate for proposal MVP
    hard_levels = {"A", "B", "D"}
    hard_ok = True
    ...
    print("\n" + "=" * 64)
    print("Note: Level C = confirm fixed pipeline deleted. Level F = proposal primary.")
    if hard_ok and f_ok:
        print("RESULT: FULL PROPOSAL MVP PASS (A/B/D + F)")
        code = 0
...
if __name__ == "__main__":
    ...
    from backends.delivery.env import delivery_root  # noqa: F401
